...
#
# Tags look like `v<Homarr version>-<release>`:
#
# - if defaults/main.yml points at a Homarr version that has never been
#   released, the release counter restarts at 0 (`v1.76.0-0`)
# - otherwise the counter is incremented (`v1.76.0-1`), but only if something
...
# Anchored on `homarr_version:` so that neither the similarly named
# `homarr_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^homarr_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Homarr version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.76.0`),
