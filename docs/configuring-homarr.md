...
>[!NOTE]
> Other type of values such as one generated with `pwgen -s 64 1` does not work. The role rejects anything that is not exactly 64 hexadecimal characters, because Homarr itself does not: it starts, keeps running, and then answers every request with an HTTP 500, while the systemd service still looks healthy. This role therefore refuses the value during validation instead.
...
homarr_database_postgres_socket_path_host: ""
```
...
homarr_database_postgres_socket_enabled: false

homarr_database_postgres_hostname: YOUR_POSTGRES_SERVER_HOSTNAME_HERE
homarr_database_postgres_port: 5432
```
